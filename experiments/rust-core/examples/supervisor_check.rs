...
impl ApiClient for Counter {
    fn on_event(&self, _e: ClientEvent) {
...
fn main() {
    ...
    println!("events received: {n}; spider up: {spider_up}; engine up: {engine_up}");
    ...
    let ok = n > 0 && spider_up && engine_up && !spider_after && !engine_after && shm_gone;
